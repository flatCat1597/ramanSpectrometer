...
module filterWheel(){
	difference(){
		...
		translate([0,-2,-26.25]) rotate([90,0,0]) cylinder(r=8,h=5,center=true);
	union(){
		translate([-26.25,-2,-0]) rotate([90,0,0]) cylinder(r=8,h=5,center=true);
		translate([-26.25,-2.5,0]) cube(size=[7,4,45],center=true);
	}
		translate([0,-3,-26.25]) rotate([90,0,0]) cylinder(r=9,h=4,center=true);
...
filterWheel();
		...
		translate([0,-2.5,-26.25]) {
//			rotate([90,0,0]) cylinder(r=8.75,h=3,center=true);
		}
